import { requireSession } from "@/lib/auth-helpers";
import { prisma } from "@/lib/prisma";
import { parseYearMonth, toDate } from "@/lib/date";

export const dynamic = "force-dynamic";

function roundToTenth(value: number) {
  return Math.round(value * 10) / 10;
}

export async function POST(request: Request) {
  const { session, response } = await requireSession();
  if (response) return response;

  const body = await request.json().catch(() => null);
  const date = body?.date;
  const entries = body?.entries;

  if (typeof date !== "string" || !Array.isArray(entries)) {
    return Response.json({ error: "invalid payload" }, { status: 400 });
  }

  const dateObj = toDate(date);
  if (!dateObj) {
    return Response.json({ error: "invalid date" }, { status: 400 });
  }

  const monthToken = date.slice(0, 7);
  if (!parseYearMonth(monthToken)) {
    return Response.json({ error: "invalid month" }, { status: 400 });
  }

  const items = entries
    .filter((entry: any) => entry && typeof entry.projectItemId === "string")
    .map((entry: any) => ({
      projectItemId: entry.projectItemId,
      value: Number(entry.value || 0)
    }));

  const total = roundToTenth(
    items.reduce((sum, entry) => sum + roundToTenth(entry.value), 0)
  );

  if (total !== 100 && total !== 0) {
    return Response.json({ error: "total must be 0 or 100" }, { status: 400 });
  }

  const itemIds = items.map((entry) => entry.projectItemId);
  if (itemIds.length > 0) {
    const allowedItems = await prisma.projectItem.findMany({
      where: { id: { in: itemIds } },
      select: { id: true }
    });
    const allowedSet = new Set(allowedItems.map((item) => item.id));
    if (itemIds.some((id) => !allowedSet.has(id))) {
      return Response.json({ error: "invalid project item" }, { status: 400 });
    }
  }

  await prisma.$transaction(async (tx) => {
    await tx.timeEntry.deleteMany({
      where: {
        userId: session.user.id,
        date: dateObj
      }
    });
    if (total > 0) {
      await tx.timeEntry.createMany({
        data: items
          .filter((entry) => roundToTenth(entry.value) > 0)
          .map((entry) => ({
            userId: session.user.id,
            date: dateObj,
            projectItemId: entry.projectItemId,
            value: roundToTenth(entry.value)
          }))
      });
    }
  });

  return Response.json({ ok: true });
}
